...
from train_utils import set_seed, save_summary  # senin önceki utils'in
...
CONFIG: Dict[str, Any] = {
    "model_name": "mobilenet_v2_from_yolo_labels",
    "epochs": 20,
    "batch_size": 32,
    "lr": 1e-3,
    "img_size": 224,
    ...
    "seed": 42,
    "run_dir": os.path.join(ROOT_DIR, "runs", "cls_mobilenet_v2_yolo"),
...
def get_dataloaders():
    ...
    # train: datasets/fire-8/train/images & labels
    train_img_dir = os.path.join(YOLO_ROOT, "train", "images")
    train_lbl_dir = os.path.join(YOLO_ROOT, "train", "labels")

    # val: datasets/fire-8/valid/images & labels
    val_img_dir = os.path.join(YOLO_ROOT, "valid", "images")
    val_lbl_dir = os.path.join(YOLO_ROOT, "valid", "labels")

    train_ds = YoloFireSmokeDataset(train_img_dir, train_lbl_dir, transform=transform)
    val_ds   = YoloFireSmokeDataset(val_img_dir,   val_lbl_dir,   transform=transform)
...
def main():
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_path = os.path.join(CONFIG["run_dir"], "best_mobilenet_v2.pth")
            torch.save(model.state_dict(), best_path)
            print("[OK] En iyi model güncellendi:", best_path)

    # YOLO ile uyumlu summary
    summary = {
        "split": "val",
