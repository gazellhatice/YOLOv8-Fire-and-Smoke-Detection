...
from train_utils import set_seed, save_summary  # senin utils'in
...
CONFIG: Dict[str, Any] = {
    "model_name": "swin_t_from_yolo_labels",
    "epochs": 25,                 # istersen 30-40 yapabilirsin
    "batch_size": 32,
    "lr": 1e-4,                   # transformer'larda genelde biraz daha düşük lr iyi
    "img_size": 224,
    "num_classes": 3,             # fire / smoke / none
    "seed": 42,
    "run_dir": os.path.join(ROOT_DIR, "runs", "cls_swin_t_yolo"),
...
def build_model():
    ...
    # IMAGENET pretrain'li Swin Transformer Tiny
    weights = models.Swin_T_Weights.IMAGENET1K_V1
    model = models.swin_t(weights=weights)

    # Swin'de classifier: model.head
...
def main():
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=CONFIG["lr"])  # transformer için AdamW güzel
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_path = os.path.join(CONFIG["run_dir"], "best_swin_t.pth")
            torch.save(model.state_dict(), best_path)
            print("[OK] En iyi Swin-T modeli güncellendi:", best_path)
