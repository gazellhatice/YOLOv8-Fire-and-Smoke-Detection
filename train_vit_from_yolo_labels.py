...
import timm  # Vision Transformer ve diğer modern modeller için

from train_utils import set_seed, save_summary 

ROOT_DIR = os.path.dirname(os.path.abspath(__file__))

# YOLO dataset kökü
# datasets/fire-8/train/images, train/labels
# datasets/fire-8/valid/images, valid/labels
YOLO_ROOT = os.path.join(ROOT_DIR, "datasets", "fire-8")

CONFIG: Dict[str, Any] = {
    "model_name": "vit_base_patch16_224_from_yolo",
    "epochs": 25,               # istersen 30-40 yapabilirsin
    "batch_size": 16,           # ViT ağır, GPU yoksa 8'e bile çekebilirsin
    "lr": 1e-4,                 # büyük model, düşük öğrenme oranı
    "img_size": 224,
    "num_classes": 3,           # fire / smoke / none
    "seed": 42,
    "run_dir": os.path.join(ROOT_DIR, "runs", "cls_vit_base_yolo"),
...
# ----------------------------------------------------
# 4) Model (Vision Transformer - ViT-B/16)
# ----------------------------------------------------
def build_model():
    num_classes = CONFIG["num_classes"]

    # timm ile ViT-B/16 (ImageNet pretrain'li)
...
def main():
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_path = os.path.join(CONFIG["run_dir"], "best_vit_base.pth")
            torch.save(model.state_dict(), best_path)
            print("[OK] En iyi ViT-B/16 modeli güncellendi:", best_path)
